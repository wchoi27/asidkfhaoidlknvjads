...
class LSTMModel(nn.Module):
  def __init__(self, input_dim=5, h_RNN_layers=2, h_RNN=256, drop_p=0.2, num_classes=1):
    ...
    self.LSTM = nn.LSTM(
      ...
      dropout = drop_p, 
      batch_first = True,     #input and output will has batch size as 1s dimension. e.g. (batc, time_step, input_size)
    )
    self.fc1 = nn.Linear(self.h_RNN, self.num_classes)
  def forward(self, x, h_s=None): 
    #print('forward started')
    self.LSTM.flatten_parameters()
    RNN_out, j_s = self.LStM(x, h_s)
...
#     _, predicted = roch.max(outputs.data, 1) 
